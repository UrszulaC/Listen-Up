...
# We need to get a token firstly
def show_token(cid, secret):
...
# After that we worked throguh the track recommendations

def show_track_recommendations(seed_tracks, token):
    limit = 10
...
# The below will enable a physical visualisation of the song recommendations.
# It uses matplotlib so we needed to make sure that they are requested at the top of the file

def song_recommendation_visualiser(reco_df):
...
# In order to save album images

def save_album_image(img_url, track_id):
    r = requests.get(img_url)
    open('img/' + track_id + '.jpg', "wb").write(r.content)

# We need the below to enable the album artwork visualisation in the app
